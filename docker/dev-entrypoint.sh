...
if [ -z "$DATABASE_URL" ]; then
  /app/server/scripts/wait-for-it.sh ${PG_HOST:-postgres}:${PG_PORT:-5432} --strict --timeout=300 -- echo "PostgreSQL is up"
else
  PG_HOST=$(echo "$DATABASE_URL" | awk -F'[/:@?]' '{print $6}')
  PG_PORT=$(echo "$DATABASE_URL" | awk -F'[/:@?]' '{print $7}')
  /app/server/scripts/wait-for-it.sh "$PG_HOST:$PG_PORT" --strict --timeout=300 -- echo "PostgreSQL is up"
fi
...
/app/server/scripts/wait-for-it.sh ${REDIS_HOST}:${REDIS_PORT} --strict --timeout=300 -- echo "Redis is up"

# Wait for PostgREST (external service)
echo "Waiting for PostgREST..."
PGRST_WAIT_HOST=$(echo "${PGRST_HOST:-postgrest}" | sed -e 's|^[^/]*//||' -e 's|:[0-9]*$||')
PGRST_WAIT_PORT=$(echo "${PGRST_HOST:-postgrest}" | grep -oE '[0-9]+$' || echo "${PGRST_SERVER_PORT:-3000}")
/app/server/scripts/wait-for-it.sh ${PGRST_WAIT_HOST}:${PGRST_WAIT_PORT} --strict --timeout=300 -- echo "PostgREST is up"
